...
UNAVAILABLE_RSP_ADDRESSES = {
    ...
    "rsp.tele2.ru",  # inaccessible
    "t2.toprsp.com",  # inaccessible
    "rsp.simhub.cn",  # inaccessible
}
...
def get_peer_certificate(host: str):
    ...
    try:
        ...
        return None
    return x509.load_pem_x509_certificate(certificate.encode()).public_bytes(
        Encoding.DER
    )
...
def store_certificate_with_openssl(file_path: str, certificate: bytes):
    ...
    certificate_options = ["ext_parse"]
    name_options = ["sep_multiline", "space_eq", "sname", "utf8"]
...
def get_records() -> tuple[set[str], set[str], list[Record]]:
    with open(RECORD_FILE, "r", newline="") as fp:
        ...
        records: list[Record] = [
            Record(row["smdp_address"], row["issuer"], row["key_id"]) for row in rows
        ]
    issuers = {record.issuer for record in records}
    issuers.difference_update(
        map(str.strip, os.environ.get("SMDP_ISSUERS", "").split(";"))
    )
    addresses = {record.smdp_address for record in records}
    addresses.difference_update(
        map(str.strip, os.environ.get("SMDP_ADDRESSES", "").split(";"))
    )
    ...
    addresses = {
        _.lower() for _ in addresses if _ and _ not in UNAVAILABLE_RSP_ADDRESSES
    }
